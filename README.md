...
5. Поле `Команда запуска` оставьте пустым: команда уже задана в `Dockerfile`.
6. Добавьте переменные окружения:
   ...
   Переменную `PORT` можно не задавать: Docker-контейнер явно запускает Gunicorn на порту `8000`.

7. Подключите постоянный диск/volume к пути:

   ```text
   /data
   ```

8. В настройках домена Timeweb Cloud привяжите `blacksquare72.ru` к этому приложению.
9. Если Timeweb Cloud выдаст CNAME или A-запись, добавьте ее в DNS-зону домена.

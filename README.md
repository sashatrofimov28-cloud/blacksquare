...
5. Поле `Команда запуска` оставьте **полностью пустым**. Не вводите пробелы.
   Если Timeweb всё равно требует команду, укажите ровно:

   ```text
   python /app/server.py
   ```

   Альтернатива (если первый вариант не сработает):
